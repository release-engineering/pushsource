...
def test_koji_poll_for_signed_rpm(mock_sleep, mock_path_exists, fake_koji, koji_dir, caplog):
    """RPM uses first existing of specified signing keys. Poll for the keys until one exists."""
    ...
    # We want True on third item, last attempt
    mock_path_exists.side_effect = [False] * 8 + [True, True]
    # Eagerly fetch
    items = list(source)
